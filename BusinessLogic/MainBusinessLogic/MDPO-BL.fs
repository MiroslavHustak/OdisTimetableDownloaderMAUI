...
module MDPO_BL = //FsHttp
            ...
            async
                {
                    ...
                    | ex 
                        -> 
                        postToLogFile (sprintf "%s Error%s" <| string ex.Message <| "#25")
    ...
    let internal safeDownloadAndSaveTimetables reportProgress (token : CancellationToken) (pathToDir : string) (filterTimetables : Map<string, string>) =  
            ...
            async
                {                      
                    try
                        ...
                        | Error err 
                            -> 
                            postToLogFile (sprintf "%s Error%s" <| err <| "#26")
                            |> Async.RunSynchronously 
                            |> ignore<ResponsePost> 
                    ...
                    | ex
                        ->
                        postToLogFile (sprintf "%s Error%s" <| string ex.Message <| "#27") 
                        |> Async.RunSynchronously 
                        |> ignore<ResponsePost> 
        ...
        let downloadTimetables reportProgress (token : CancellationToken) : Result<unit, MHDErrors> = 
                ...
                let l = filterTimetables |> Map.count
                    ...
                    |> List.tryPick
                        (function
                            ...
                            | Error err
                                ->
                                postToLogFile (sprintf "%s Error%s" <| string err.Message <| "#28") 
                                |> Async.RunSynchronously
                                |> ignore<ResponsePost>  
            ...
            | ex    
                ->
                postToLogFile (sprintf "%s Error%s" <| string ex.Message <| "#281") 
                |> Async.RunSynchronously 
                |> ignore<ResponsePost>  
                ...
                let dirName = ODISDefault.OdisDir6                       
                    ...
                    | Ok _    -> 
                              postToLogFile (sprintf "%s Error%s" <| string FileDownloadErrorMHD<| "#282") 
                              |> Async.RunSynchronously
                              |> ignore<ResponsePost>
                              Error FileDownloadErrorMHD
                    | Error _ -> 
                              postToLogFile (sprintf "%s Error%s" <| string FileDeleteErrorMHD <| "#283") 
                              |> Async.RunSynchronously 
                              |> ignore<ResponsePost>
            ...
            async
                {
                    ...
                    | ex 
                        -> 
                        postToLogFile (sprintf "%s Error%s" <| string ex.Message <| "#29")
                        |> Async.RunSynchronously 
                        |> ignore<ResponsePost>  
    ...
    let internal unsafeDownloadAndSaveTimetables reportProgress (token : CancellationToken) (pathToDir : string) (filterTimetables : Map<string, string>) =  
            ...
            async
                {                      
                    try  
                        ...
                        | Error err 
                            -> 
                            postToLogFile (sprintf "%s Error%s" <| err <| "#30") 
                            |> Async.RunSynchronously
                            |> ignore<ResponsePost> 
                    ...
                    | ex
                        ->
                        postToLogFile (sprintf "%s Error%s" <| string ex.Message <| "#31")
                        |> Async.RunSynchronously 
                        |> ignore<ResponsePost>  
        ...
        let downloadTimetables reportProgress (token : CancellationToken) : Result<unit, MHDErrors> = 
                ...
                let l = filterTimetables |> Map.count
                    ...
                    |> List.tryPick
                        (function
                            ...
                            | Error err
                                ->
                                postToLogFile (sprintf "%s Error%s" <| string err.Message <| "#32") 
                                |> Async.RunSynchronously 
                                |> ignore<ResponsePost> 
            ...
            | ex   
                ->
                postToLogFile (sprintf "%s Error%s" <| string ex.Message <| "#33") 
                |> Async.RunSynchronously
                |> ignore<ResponsePost> 
                ...
                let dirName = ODISDefault.OdisDir6                       
                    ...
                    | Ok _   
                        -> 
                        postToLogFile (sprintf "%s Error%s" <| string FileDownloadErrorMHD <| "#331") 
                        |> Async.RunSynchronously 
                        |> ignore<ResponsePost>
                        Error FileDownloadErrorMHD
                    | Error _ 
                        -> 
                        postToLogFile (sprintf "%s Error%s" <| string FileDeleteErrorMHD <| "#332")
                        |> Async.RunSynchronously
                        |> ignore<ResponsePost>
                        Error FileDeleteErrorMHD 
